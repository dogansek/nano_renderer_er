...
Workflow:
1. Capture Viewport - Captures clean viewport WITHOUT AI processing (FREE)
2. Generate - Create images based on Primary Reference with AI (COSTS API USAGE)
3. Iterate - Replace Primary Reference with result (continues conversation)

Version: 4.2 - No-Prompt Capture Edition
Date: October 2025
...
class NanoBananaChatForm(Forms.Form):
    ...
    def _setup_widgets(self):
        ...
        self._prompt_ever_focused = False
        self._prompt_placeholder_text = """Capture active viewport to start creating!

Quick Guide:
• Capture: Clean viewport capture (FREE - no AI processing)
• Generate: AI image generation with your prompt + mood board
• Iterate: Move generated variation to primary reference for further refinement"""
        ...
        def on_prompt_focus(sender, e):
            if self._prompt_placeholder_active and self.prompt_tb.Enabled:
                ...
                self._prompt_ever_focused = True
        ...
        self.iterate_btn.Enabled = False
    ...
    def _build_reference_controls(self):
        """Build mood board image controls with previews."""
        self.ref_previews = []
    ...
    def _create_render_tab(self):
        ...
        img_w = col_w
        img_h = int(round(img_w * 9.0 / 16.0))
        ...
        for i in range(4):
            try:
                ...
                if hasattr(self.ref_preview_panels[i], 'Size'):
                    self.ref_preview_panels[i].Size = Drawing.Size(img_w, img_h + 6)
...
Anthropic Claude
    ...
    def on_capture_viewport(self, sender, event):
        """Handle capture viewport button - CAPTURES CLEAN VIEWPORT WITHOUT AI PROCESSING."""
    ...
    def _execute_capture_viewport(self):
        """Execute the actual viewport capture."""
        try:
            ...
            # Update the Primary Reference preview with the captured viewport
            self._update_viewport_preview(bitmap)
            
            # ENABLE PROMPT BOX NOW THAT VIEWPORT IS CAPTURED
            self._update_prompt_state()
            
            # Enable generate button (iterate stays disabled until first generation)
            self.generate_btn.Enabled = True
            
            self._append_chat_log("Viewport captured successfully! Ready to generate.")
            
        except Exception as e:
            self._append_chat_log(f"Failed to capture viewport: {e}")
    ...
    def on_generate(self, sender, event):
        ...
        # BUILD CONTENT PARTS WITH PRIMARY REFERENCE → PROMPT → MOOD BOARD ORDER
        parts = []
